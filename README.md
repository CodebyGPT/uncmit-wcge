...
| `scr/uncmit.ps1` | 交互式 GUI 替换脚本（PowerShell 7 only，自动禁用 WPBT 执行） |
| `build/*.clean.ps1` | 去 CMIT 的 PowerShell 源（可审计，含 `# UNCMIT-DISABLED` 标记） |
| `reverse/*.extracted.ps1` | 从原版 exe 逆向提取的 PowerShell（分析用） |
| `scr/uncmit-cleanup.cmd` | 后期清理脚本（可选）：首次登录后以管理员身份运行，清除惰性 CMIT 载荷 |
| `docs/exe-locations.md` | 6 个 exe 在 WIM 中的路径与 Disable/Preserve 分类 |
| `docs/audit-report.md` | 原版 ISO 完整审计报告 |
| `docs/memories/` | 项目知识库：cmd 脚本陷阱、编译规范、项目布局、GDCA 证书指纹 |
| `samples/Windows/EnterpriseG.xml` | 原版 Windows 10 EnterpriseG 版本配置（Edition Configuration） |
...
2. 以管理员身份打开 PowerShell 7，执行以下**一行命令**：
   ```powershell
   irm https://raw.githubusercontent.com/CodebyGPT/uncmit-wcge/master/scr/uncmit.ps1 -OutFile $env:TEMP\uncmit.ps1; & $env:TEMP\uncmit.ps1
   ```
...
如果要从源码重新编译部署 exe，见 `docs/memories/build-guidelines.md` 了解
PS2EXE 参数规范（`-noConsole -noOutput -requireAdmin`，禁止 `-noError`）。

## 安全说明

- **WPBT 防御**：修改 WIM 时自动写入 `DisableWpbtExecution=1` 注册表键，阻止固件在引导早期强制执行二进制文件。`uncmit-cleanup.cmd` 的 SECTION 6 提供后置检测与用户选择禁用。
- **只禁调用，不删除二进制**：被调用的闭源二进制（EPrivilege.exe、CMGEInstaller.exe、
  CmitClient.exe 等）仍保留在镜像中作为死文件，仅移除其触发逻辑。这符合"最小改动"原则。
- **证书隐私优先**：部署阶段已跳过所有 CMIT 证书导入（CMIT Root Authority、CMIT SubP、
  CMIT signature 及政府/行业 CA），系统证书存储区不残留任何 CMIT 闭源信任锚点。
...
- 可选：删除 CKB2024103103 引入的 GDCA 根证书（确保隐私优先策略，询问 y/N）

```cmd
:: 以管理员身份运行（双击即可，无需 PowerShell）
:: 或手动下载后右键 → 以管理员身份运行
curl.exe -LO https://raw.githubusercontent.com/CodebyGPT/uncmit-wcge/master/scr/uncmit-cleanup.cmd
uncmit-cleanup.cmd
```
