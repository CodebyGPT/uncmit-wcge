...
# UNCMIT-DISABLED Set-RegistryValue -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ClipSVC" -Name "AltActivationClient" -Value "%ProgramFiles%\CMITActivation\CmitClient.exe" -PropertyType "String"
...
#CMITCMGEInstaller
# UNCMIT-DISABLED New-Item $env:windir\Temp\cmit -ItemType Directory -Force
# UNCMIT-DISABLED Copy-Item "$env:SystemDrive\Windows.old\Windows\System32\config\SOFTWARE" "$env:windir\Temp\cmit\" -Force >> $logCMD
# UNCMIT-DISABLED Copy-Item "$env:SystemDrive\Windows.old\Windows\INF\cua.ini" "$env:windir\Temp\cmit\" -Force >> $logCMD
# UNCMIT-DISABLED Start-Process -FilePath "$env:SystemDrive\Recovery\OEM\CMGE\ResetSources\EPrivilege.exe" -ArgumentList " -U:S $env:SystemDrive\Recovery\OEM\CMGE\ResetSources\CMGEInstaller\CMGEInstaller.exe 00000200" -WindowStyle Hidden -Wait
...
# UNCMIT-DISABLED $Action = New-ScheduledTaskAction -Execute "$env:windir\Temp\UpgradeSchdTask.exe"
# UNCMIT-DISABLED $Trigger = New-ScheduledTaskTrigger -AtLogon
# UNCMIT-DISABLED $Principal = New-ScheduledTaskPrincipal -GroupID "Builtin\Users" -RunLevel Highest
# UNCMIT-DISABLED $Settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -ExecutionTimeLimit (New-TimeSpan -Seconds 60) -Compatibility Win8
# UNCMIT-DISABLED $SchTask = New-ScheduledTask -Action $Action -Principal $Principal -Trigger $Trigger -Settings $Settings
# UNCMIT-DISABLED Register-ScheduledTask "UpgradeSchdTask" -InputObject $SchTask
...
# UNCMIT-DISABLED $CUAExe = "cmd.exe"
# UNCMIT-DISABLED $CUACfg = "/c `"sc.exe start CmitUpdateAgent`" >> $logCMD"
# UNCMIT-DISABLED Start-Process -WindowStyle Hidden -FilePath "$CUAExe" -ArgumentList "$CUACfg" -Verb runas -Wait
Start-Sleep -Seconds 2
