...
internal class CredentialForm
{
	...
	internal static UserPwd PromptForPassword(string caption, string message, string target, string user, PSCredentialTypes credTypes, PSCredentialUIOptions options)
	{
		...
		StringBuilder stringBuilder2 = new StringBuilder(user);
		...
		if (CredUIPromptForCredentials(ref creditUR, target, IntPtr.Zero, 0, stringBuilder2, 100, stringBuilder, 100, ref pfSave, cREDUI_FLAGS) == CredUIReturnCodes.NO_ERROR)
		{
